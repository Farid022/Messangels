...
struct FuneralMusicNote: View {
    ...
    var body: some View {
        FuneralNoteAttachCutomActionView(totalSteps: 4.0, showNote: $showNote, note: $vm.music.broadcast_song_note, loading: $loading, oldAttachedFiles: $vm.music.broadcast_song_note_attachments, noteAttachmentIds: $vm.music.broadcast_song_note_attachment, menuTitle: wishesCeremony.last!.name, title: title) {
            loading.toggle()
            if vm.updateRecord {
